...
do
	local newnode = layers_mod.clone_node('air')
...
function mod.generate_floaters(params)
	...
	local ground_noise_map = layers_mod.get_noise2d('floaters_over', {x=csize.x, y=csize.z}, { x = minp.x, y = minp.z })
	local under_noise_map = layers_mod.get_noise2d('floaters_under', {x=csize.x, y=csize.z}, { x = minp.x, y = minp.z })
	local base_noise_map = layers_mod.get_noise2d('floaters_base', {x=csize.x, y=csize.z}, { x = minp.x, y = minp.z })
	...
	for z = minp.z, maxp.z do
		for x = minp.x, maxp.x do
			...
			--local sfc=params.share.surface[z][x]
			...
			local depth_filler = biome.filler_depth or 0  -- 1?
			local depth_top = biome.top_depth or 0  -- 6?!
